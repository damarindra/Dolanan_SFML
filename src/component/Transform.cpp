...
void Transform::set_global_position(sf::Vector2f v) {
	if(dln::ECS::is_entity_valid(parent)){
		const sf::Transform inverse = dln::ECS::get_registry().get<Transform>(parent).get_global_transform().getInverse();
...
void Transform::set_parent(entt::entity p_parent) {
	if(dln::ECS::is_entity_valid(p_parent) && !dln::ECS::Instance()->registry.has<Transform>(p_parent)){
	...
	if(dln::ECS::is_entity_valid(parent)){
		...
		tr.children.erase(std::remove(tr.children.begin(), tr.children.end(), owner));
	}
	parent = p_parent;
	Transform& parent_tr = dln::ECS::get_registry().get<Transform>(p_parent);
	parent_tr.children.push_back(owner);
